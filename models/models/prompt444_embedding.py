...
class Prompt(BertPreTrainedModel):
    ...
    def __init__(self, config,tokenizer=None, max_depth = 1,num_labels=1, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None, max_seg_num=None, max_seq_len=None, device=None, **kwargs):
        ...
        # self.affine = nn.Linear(in_features=2*self.bertConfig.hidden_size, out_features=self.bertConfig.hidden_size)
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
        self.init_weights()
        self.embedding = torch.Tensor().to(device)
    ...
    def forward(
        ...
        # node_pair1 = np.loadtxt('./response.txt')
        # node_pair = []
        ...
        # print('segment size:',segment.size(),'response size:',response.size(),'segment_turnmask:',segment_mask.size())
        #Segment Weighting
        ...
        # print('prediction scores:',prediction_scores.size(),prediction_scores)
        ...
        if labels is not None:
            ...
            # labels[labels==-100] = 0
            # multiclass_logits[multiclass_logits<0] = 0
            # multiclass_loss = 0
            # multiclass_logits = multiclass_logits.view(num,-1)
            # labels = labels.float()
            # losses = []
            # for label_idx in range(self.max_depth*self.num_labels):
            #     label_logits = multiclass_logits[:, label_idx]  # 提取当前标签的模型输出
            #     probabilities = torch.sigmoid(label_logits)  # 经过Sigmoid函数得到概率值
            #     # print('probabilities',probabilities)
            #     label_targets = labels[:, label_idx]  # 提取当前标签的真实标签
            #     # print('label_targets',label_targets)
            #     label_loss = nn.BCELoss(reduction='sum')(probabilities, label_targets)  # 计算BCELoss
            #     losses.append(label_loss)
            # multiclass_loss = torch.sum(torch.stack(losses)) 
        ...
        torch.save(self.embedding,'embedding/contra/contra'+str(i)+'.pt')
    ...
    @torch.no_grad()
    def generate(self, input_ids,attention_mask,token_type_ids,seg_input_ids, seg_token_type_ids, seg_attention_mask, cls_sep_pos, true_len, labels, depth2label, **kwargs):
        ...
        multiclass_pos = seg_input_ids == (self.get_input_embeddings().size - 1)
        ...
        for scores in prediction_scores:
            ...
            # print('predict_labels size:',predict_labels[-1])
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, Q, K, V,V_mask, episilon=1e-8):
        ...
        '''
        # (select_seg_context,segment_mask_seg,res_sequence_output_seg,response_mask_seg)
        # response_seg,select_seg_context,select_seg_context,segment_mask_seg
        ...
        # Q_K_score = F.softmax(Q_K, dim=-1)  # (batch_size, max_r_words, max_u_words) 80*350*350

        ## pic_attention
        # torch.save(Q_K_score,'pic/fanxiang_attention_weight.pt')
        # print(Q_K_score.size())
        # print('torch saved')
