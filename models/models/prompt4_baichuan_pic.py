...
from .modeling_baichuan import BaiChuanForCausalLM, BaiChuanOnlyMLMHead,Model
from .modeling_bert import BertPreTrainedModel, BertOnlyMLMHead, BertModel
...
class GraphEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.device)
        x = F.embedding(x.long(), self.weight().long(), self.padding_idx)
...
class Prompt(BertPreTrainedModel):
    ...
    def __init__(self, config=None,tokenizer=None, max_depth = 1,num_labels=1, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None, max_seg_num=None, max_seq_len=None, device=None, **kwargs):
        
        config.vocab_size = tokenizer.vocab_size
        # print("config",config)
        baichuanConfig = BaiChuanConfig.from_pretrained(f'/data/fkj2023/pretrained_model/baichuan-7B',local_files_only=True)
        # print("baichuanConfig vocab_size",baichuanConfig.vocab_size)
        # config.hidden_size = baichuanConfig.hidden_size
        # print("baichuanConfig:",baichuanConfig)
        baichuanConfig.output_attentions = False
        baichuanConfig.output_hidden_states = False
        baichuanConfig.attention_probs_dropout_prob = 1
        baichuanConfig.hidden_dropout_prob = 1
        baichuanConfig.num_labels
        self.bertConfig = BertConfig.from_pretrained(r'/data/zyl2023/bert-base-chinese',local_files_only=True)
        print("self.bertConfig",self.bertConfig)
        ...
        # self.baichuanConfig = BertConfig.from_pretrained(r'/data/zyl2023/bert-base-chinese',local_files_only=True)
        self.baichuanConfig = baichuanConfig
        self.baichuanConfig.hidden_size = config.hidden_size
        self.baichuanConfig.layer_norm_eps = 1e-12
        self.cls = BaiChuanOnlyMLMHead(baichuanConfig)
        
        # print("baichuanConfig",self.baichuanConfig)
        # self.bert = BertModel(BertConfig.from_pretrained(r'/data/zyl2023/bert-base-chinese',local_files_only=True))
        self.baichuan = Model(self.baichuanConfig)
        self.dropout = nn.Dropout(baichuanConfig.attention_probs_dropout_prob)
        self.alpha = 0.5
        self.linear_word = nn.Linear(2 * max_seq_len, 1) ##max_Seq_len
        self.W_word = nn.Parameter(data=torch.Tensor(self.baichuanConfig.hidden_size, self.baichuanConfig.hidden_size, max_seg_num))
        self.v = nn.Parameter(data=torch.Tensor(max_seg_num, 1))
        self.transformer_ur = TransformerBlock(device=device,input_size=self.baichuanConfig.hidden_size)
        self.transformer_ru = TransformerBlock(device=device,input_size=self.baichuanConfig.hidden_size)
        self.key_trans = nn.Linear(in_features=2*self.baichuanConfig.hidden_size, out_features=2*self.baichuanConfig.hidden_size)
        self.utt_gru_acc = nn.GRU(input_size=2*self.baichuanConfig.hidden_size, hidden_size=2*self.baichuanConfig.hidden_size, batch_first=True)
        self.affine_out = nn.Linear(in_features=4*self.baichuanConfig.hidden_size, out_features=self.baichuanConfig.hidden_size)
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
        self.init_weights()
        # print("init weights")
    ...
    def get_output_embeddings(self):
        # print("get self.cls.predictions",self.cls.predictions.decoder)
        return self.cls.predictions.decoder

    def set_output_embeddings(self, new_embeddings):
        self.cls.predictions.decoder = new_embeddings
        # print("set self.cls.predictions",self.cls.predictions.decoder)
    

    def init_embedding(self):
        depth = len(self.depth2label)
        label_dict = torch.load(os.path.join(self.data_path, 'value_dict.pt'))
        tokenizer = AutoTokenizer.from_pretrained(r'/data/fkj2023/pretrained_model/baichuan-7B',local_files_only=True,trust_remote_code=True)
        label_dict = {i: tokenizer.encode(v) for i, v in label_dict.items()}
        label_emb = []
        input_embeds = self.baichuan.get_input_embeddings()      # bertModel的input_embedding
        # print("baichuan input_embeds:",input_embeds)
        for i in range(len(label_dict)):
            label_emb.append(
                input_embeds.weight.index_select(0, torch.tensor(label_dict[i], device=input_embeds.weight.device)).mean(dim=0))
        ...
        prefix = input_embeds(torch.tensor([tokenizer.unk_token_id],
                                           device=input_embeds.weight.device, dtype=torch.long))
        # print("prefix",prefix.device)   #1 768
        # prompt
        ...
        # print("torch.stack(label_emb): ",torch.stack(label_emb).device)
        # print("prompt_embedding.weight[1:, :]",prompt_embedding.weight[1:, :].device)
        label_emb = torch.cat([torch.stack(label_emb), prompt_embedding.weight[1:, :].to(input_embeds.weight.device), prefix], dim=0)
        # 141(label_num) + 3(prompt_embedding max_depth + 1) + 1(prefix)
        
        embedding = GraphEmbedding(self.baichuanConfig, input_embeds, label_emb, self.graph_type,
                                   path_list=self.path_list, layer=self.layer, data_path=self.data_path)
        
        self.baichuan.set_input_embeddings(embedding)
        # print("self.get_output_embeddings().bias",self.get_output_embeddings().bias.size())
        output_embeddings = OutputEmbedding(self.baichuanConfig, self.get_output_embeddings().bias)
    ...
    def get_layer_features(self, layer, prompt_feature=None):
        ...
        label_features = torch.dropout(F.relu(label_features), train=self.training, p=self.baichuanConfig.hidden_dropout_prob)
    ...
    def forward(
        ...
        multiclass_pos = input_ids == (self.baichuan.get_input_embeddings().size - 1)                      #获得CLS标记位置
        # print('multiclass_pos:',multiclass_pos.size(),multiclass_pos)                                                  #20 512
        single_labels = input_ids.masked_fill(multiclass_pos | (input_ids == self.baichuanConfig.pad_token_id), -100) #[mask]
        # print('single_labels:',single_labels.size())                                                      #CLS & padding token变为-100
        
        
        b, s = seg_input_ids.size()
        n = cls_sep_pos.size()[1] - 2
        segment = torch.zeros(b,n,s)
        print('segment size:',segment.size())
        # zeros(b,n,s).to(self.device)         #8 10 350
        response = torch.zeros((b,s))
        # zeros(b, s).to(self.device)         #8 350
        
        print('response size:',response.size())
        

        for bind,seq in enumerate(input_ids):
            ...
            for posind,pos in enumerate(cls_seq_pos_temp):  
                ...
                if(posind==true_len[bind]-2):                   #response
                    response[bind][0:m] = input_ids[bind][cls_seq_pos_temp[posind] + 1:cls_seq_pos_temp[posind + 1]]
                    
                    # response[bind][0:m] = self.tokenizer.convert_ids_to_tokens(input_ids[bind][cls_seq_pos_temp[posind] + 1:cls_seq_pos_temp[posind + 1]])
                    # print('response after:',bind,m,response[bind])
                    # response_mask[bind][0:m]= 1
                else:                                           #context segment
                    segment[bind][posind][0:m]=\
                        input_ids[bind][cls_seq_pos_temp[posind]+1:cls_seq_pos_temp[posind+1]]
                    
                    # segment[bind][posind] = self.tokenizer.convert_ids_to_tokens(input_ids[bind][cls_seq_pos_temp[posind]+1:cls_seq_pos_temp[posind+1]])
                    # print("segment after:",bind,posind,m,segment[bind][posind][0:m])
                    # segment_mask[bind][posind][0:m] =1
                    # segment_turnmask[bind][posind]=1
        
        print('segment size:',segment.size)
        segment=segment.view(b*n,s)                     #80 350 
        
        torch.save(segment,'pic/segment.pt')

        # response_tensor = torch.tensor([[ord(c) for c in row] for row in response])
        response = response.unsqueeze(dim=1).repeat(1, n, 1)
        response = response.view(b*n,s)           #80 350

        torch.save(response,'pic/response.pt')
        ...
        if self.training:
            ...
            input_ids = input_ids.masked_fill(random_mask > 0.865, self.tokenizer.unk_token_id)
            ...
            self.baichuan.training = True
        num = single_labels.shape[0]
        multiturn_labels = torch.full((num,1),-100).to(input_ids.device)
        single_labels = torch.cat((single_labels,multiturn_labels),dim=1)
        multirturn_pos =  torch.full((num,1),False).to(input_ids.device)
        ...
        if labels is not None:
            ...
            # print("prediction_scores",prediction_scores.dtype)
            single_labels  = torch.tensor(single_labels, dtype=torch.int64)
            # print("single_labels",single_labels.dtype)
            ...
            # print("multiclass_loss",multiclass_loss)
    ...
    def prepare_inputs_for_generation(self, input_ids, attention_mask=None, **model_kwargs):
        ...
        #  add a dummy token
        assert self.baichuanConfig.pad_token_id is not None, "The PAD token should be defined for generation"
        attention_mask = torch.cat([attention_mask, attention_mask.new_zeros((attention_mask.shape[0], 1))], dim=-1)
        dummy_token = torch.full(
            (effective_batch_size, 1), self.baichuanConfig.pad_token_id, dtype=torch.long, device=input_ids.device
        )
    ...
    def word_selector(self, key, context,segment_turnmask):
        ...
        self.v max seg len
        '''
        dk = torch.sqrt(torch.Tensor([self.baichuanConfig.hidden_size])).to(self.device)
