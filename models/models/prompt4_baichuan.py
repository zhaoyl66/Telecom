...
class GraphEmbedding(nn.Module):
    ...
    def forward(self, x):
        print("type(self.weight())",type(self.weight()))
        print(type(self.padding_idx))
...
class Prompt(BertPreTrainedModel):
    ...
    def __init__(self, config=None,tokenizer=None, max_depth = 1,num_labels=1, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None, max_seg_num=None, max_seq_len=None, device=None, **kwargs):
        ...
        # self.baichuan = BaiChuanForCausalLM(self.baichuanConfig).cuda()
        ...
        self.transformer_ur = TransformerBlock(input_size=self.baichuanConfig.hidden_size,device=device)
        self.transformer_ru = TransformerBlock(input_size=self.baichuanConfig.hidden_size,device=device)
    ...
    def init_embedding(self):
        ...
        label_emb = torch.cat([torch.stack(label_emb), prompt_embedding.weight[1:, :].to(self.device), prefix], dim=0)
    ...
    def forward(
        ...
        # print(single_labels.device,multiturn_labels.device)
        ...
        # prediction_scores = self.cls(sequence_output)       #CLS位置 分类scores 16 350 768
        ...
        # print("segment_turnmask",segment_turnmask.size())
    ...
    def generate(self, input_ids,attention_mask,token_type_ids,seg_input_ids, seg_token_type_ids, seg_attention_mask, cls_sep_pos, true_len, depth2label, **kwargs):
        ...
        multirturn_pos =  torch.full((num,1),False)
        multiclass_pos = torch.cat((multirturn_pos,multiclass_pos),dim=1)
    ...
    def word_selector(self, key, context,segment_turnmask):
        ...
        # print("context",context.size())             #1, 6, 16, 64000
        # print("self.W_word",self.W_word.size())     #4096, 4096, 6
        # print("key",key.size())                     #1, 16, 64000
        # print("dk",dk.size())                       #1
        # print("context",context.device)
        # print("self.W_word",self.W_word.device)
        # print("key",key.device)
        # print("dk",dk.device)
    ...
    def utterance_selector(self, key, context,segment_turnmask):
        ...
        print("context",context.size())                                         #1, 6, 16, 64000
        print("key",key.size())                                                 #1, 16, 64000
        print("segment_turnmask",segment_turnmask.size())                       #1
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self,input_size, device, is_layer_norm=False):
