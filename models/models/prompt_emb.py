...
class Prompt(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]                            #last_hidden_state 16 512 768
        # prediction_scores = self.cls(sequence_output)       #CLS位置 分类scores
        # save.append(list(zip(sequence_output,labels)))
        self.emb.append(sequence_output)                      # save token embedding
        # print(self.emb)
        print("*********")
        with open("en_data/train/train_"+str(i)+".pkl","wb") as fp:
            # pkl.dump(zip(self.emb,labels),fp)
            pkl.dump(self.emb,fp)
        self.emb = []
