...
class Prompt(BertPreTrainedModel):
    ...
    def __init__(self, config,tokenizer=None, max_depth = 1,num_labels=1, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None, max_seg_num=None, max_seq_len=None, device=None, **kwargs):
        ...
        self.affine_out = nn.Linear(in_features=4*self.bertConfig.hidden_size, out_features=1)
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
    ...
    def init_embedding(self):
        ...
        # print("output embeddings size", self.get_output_embeddings().bias.size())
    ...
    def forward(
        ...
        # print('V_seg size:', V_seg.size(), V_seg)  #16*10 350 1536
        # loaded_tensor = torch.load('V_seg.pt')
        # print("**************V_seg")
        # print(loaded_tensor.size(),loaded_tensor)
        # loaded_tensor = loaded_tensor.to(V_seg.device)
        # print(torch.equal(loaded_tensor, V_seg))
        ...
        logits = torch.sigmoid(L_seg_key).squeeze(dim=-1)
    ...
    @torch.no_grad()
    def generate(self, input_ids, seg_token_type_ids, seg_attention_mask, cls_sep_pos, true_len, labels, depth2label, **kwargs):
        ...
        '''
        prediction_scores = multiclass_logits
