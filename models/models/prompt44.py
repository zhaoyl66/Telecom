...
class Prompt(BertPreTrainedModel):
    ...
    def forward(
        ...
        torch.save(response,'response.pt')
        segment = tokenizer.convert_ids_to_tokens(segment)
        print("segment",segment)
        torch.save(segment,'segment.pt')
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, Q, K, V,V_mask, episilon=1e-8):
        ...
        # K.premute(0,2,1) batch_size* embedding_dim* max_u_words
        Q_K = Q.bmm(K.permute(0, 2, 1)) / (torch.sqrt(dk) + episilon) #(batch_size, max_r_words, max_u_words)
        
        Q_K_score=masked_softmax(Q_K,V_mask[:,None,:])  #attention_weight
        # Q_K_score = F.softmax(Q_K, dim=-1)  # (batch_size, max_r_words, max_u_words)
        torch.save(Q_K_score,'attention_weight.pt')
