...
from .modeling_chatglm import ChatGLMModel, GLMOnlyMLMHead
...
# class Prompt(ChatGLMModel):
    ...
    def __init__(self, config=None,tokenizer=None, max_depth = 1,num_labels=1, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None, max_seg_num=None, max_seq_len=None, device=None, **kwargs):
        config.hidden_size = self.glmConfig.hidden_size
        super().__init__(config)
        glmConfig = ChatGLMConfig.from_pretrained(f'/data/fkj2023/pretrained_model/chatglm2-6b',local_files_only=True)
        glmConfig.output_attentions = False
        glmConfig.output_hidden_states = False
        # glmConfig.attention_probs_dropout_prob = 1
        # glmConfig.hidden_dropout_prob = 0.1
        ...
        self.glmConfig.layer_norm_eps = 1e-12
        self.cls = GLMOnlyMLMHead(glmConfig)
        self.glm = ChatGLMModel(self.glmConfig)
        self.dropout = nn.Dropout(glmConfig.attention_probs_dropout_prob)
        self.alpha = 0.5
        self.linear_word = nn.Linear(2 * max_seq_len, 1) ##max_Seq_len
        self.W_word = nn.Parameter(data=torch.Tensor(self.glmConfig.hidden_size, self.glmConfig.hidden_size, max_seg_num))
        self.v = nn.Parameter(data=torch.Tensor(max_seg_num, 1))
        self.transformer_ur = TransformerBlock(device=device,input_size=self.glmConfig.hidden_size)
        self.transformer_ru = TransformerBlock(device=device,input_size=self.glmConfig.hidden_size)
        self.key_trans = nn.Linear(in_features=2*self.glmConfig.hidden_size, out_features=2*self.glmConfig.hidden_size)
        self.utt_gru_acc = nn.GRU(input_size=2*self.glmConfig.hidden_size, hidden_size=2*self.glmConfig.hidden_size, batch_first=True)
        self.affine_out = nn.Linear(in_features=4*self.glmConfig.hidden_size, out_features=self.glmConfig.hidden_size)
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
        self.init_weights()
        self.embedding = []
    ...
    def init_embedding(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained(r'/data/fkj2023/pretrained_model/chatglm2-6b',local_files_only=True,trust_remote_code=True)
        label_dict = {i: tokenizer.encode(v) for i, v in label_dict.items()}
        label_emb = []
        input_embeds = self.glm.get_input_embeddings()      # bertModel的input_embedding
        ...
        embedding = GraphEmbedding(self.glmConfig, input_embeds, label_emb, self.graph_type,
                                   path_list=self.path_list, layer=self.layer, data_path=self.data_path)
        
        self.glm.set_input_embeddings(embedding)
        output_embeddings = OutputEmbedding(self.glmConfig, self.get_output_embeddings().bias)
    ...
    def get_layer_features(self, layer, prompt_feature=None):
        ...
        label_features = self.glm.get_input_embeddings().new_embedding(labels)
        label_features = self.transform(label_features)
        label_features = torch.dropout(F.relu(label_features), train=self.training, p=self.glmConfig.hidden_dropout_prob)
    ...
    def forward(
        ...
        multiclass_pos = input_ids == (self.glm.get_input_embeddings().size - 1)                      #获得CLS标记位置
        # print('multiclass_pos:',multiclass_pos.size(),multiclass_pos)                                                  #20 512
        single_labels = input_ids.masked_fill(multiclass_pos | (input_ids == self.glmConfig.pad_token_id), -100) #[mask]
    ...
    def prepare_inputs_for_generation(self, input_ids, attention_mask=None, **model_kwargs):
        ...
        #  add a dummy token
        assert self.glmConfig.pad_token_id is not None, "The PAD token should be defined for generation"
        attention_mask = torch.cat([attention_mask, attention_mask.new_zeros((attention_mask.shape[0], 1))], dim=-1)
        dummy_token = torch.full(
            (effective_batch_size, 1), self.glmConfig.pad_token_id, dtype=torch.long, device=input_ids.device
        )
    ...
    def generate(self, input_ids,attention_mask,token_type_ids,seg_input_ids, seg_token_type_ids, seg_attention_mask, cls_sep_pos, true_len, depth2label, **kwargs):
        ...
        multiclass_pos = input_ids == (self.glm.get_input_embeddings().size - 1)
    ...
    def word_selector(self, key, context,segment_turnmask):
        ...
        self.v max seg len
        '''
        dk = torch.sqrt(torch.Tensor([self.glmConfig.hidden_size])).to(self.device)
